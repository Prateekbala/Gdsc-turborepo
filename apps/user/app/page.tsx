...
export default function Home() {
  ...
  const handleMenuClick = (item: string) => {
    if (item === "Log Out") {
      signOut({ callbackUrl: `${process.env.USER_URL}`});
    } else if (item === "Log In") {
      signIn();
    }
    else if(item==="ADMIN"){
  ...
  return (
    ...
    <nav className="flex flex-wrap items-center justify-between p-3 bg-[#eeeee9]">
      ...
      <div className="hidden sm:flex items-center justify-end mx-2">
        ...
        <Button variant="contained" className="px-4 py-2 bg-[#de8cde] text-white" onClick={async()=>{router.push("/sign-up")}}>Sign-Up</Button>
      </div>
